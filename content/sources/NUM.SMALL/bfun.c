...
#define Maxlen(x, y) ((x)->len > (y)->len ? (x)->len : (y)->len)
#define Sumlen(x, y) ((x)->len + (y)->len)
...
value negated(x) register value x; {
	...
	return mk_exact(-Numerator(x), Denominator(x), x->len);
}
...
value inverse(x) value x;{
	...
	return mk_exact(Denominator(x), Numerator(x), x->len);
}
...
value absval(x) register value x; {
	...
	return mk_exact((integer) fabs((double) Numerator(x)), Denominator(x), x->len);
}
...
value round0(x) register value x; {
	return mk_int(floor(numval(x) + .5));
}

value round(n, x) register value n, x; {
	...
	xtenp= prod(x, tenp);
	r0= round0(xtenp);
	r= mk_exact(Numerator(r0), Numerator(tenp), propintlet(Numerator(n)));
