/* B numbers */
	...
	/*
	 * THIS VERSION SHOULD BE USED ONLY IF
	 * THE SYSTEM IS TOO LARGE OTHERWISE.
	 * IT USES FLOATING POINT ARITHMETIC
	 * INSTEAD OF ARBITRARY LENGTH RATIONAL
	 * ARITHMETIC FOR EXACT NUMBERS.
	 */

#include "b.h"
#include "bnum.h"
#include "bobj.h"
#include "bsyn.h" /* for def of Keymark() only */
#include "bnuM.h"

extern value mk_int();

int propintlet(i) int i; {
...
value b_zero, b_one, b_minus_one;
...
#define BIG 72057594037927935.0
#define LONG 9999999999999999.5
#define MAXNUMDIG 16
#define MINNUMDIG  6
...
conv:	if (!prec_loss && Exact(v) && fabs(x) <= LONG &&
		...
		intlet len= 0 < v->len && v->len <= MAXNUMDIG ? v->len : 0;
