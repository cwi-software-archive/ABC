	...
	register environ *ep;
{
	...
	case FHOLE:
		ritevhole(ep);
		if (ep->mode != FHOLE && ep->mode != VHOLE)
			leftvhole(ep);
	}
	...
	register environ *ep;
{
	...
	for (i = ep->s1; i > 1 && isnull(n, rp, i-1); --i)
		;
	ep->s1 = i;
	for (i = ep->s2; i < nch21 && isnull(n, rp, i+1); ++i)
		;
	ep->s2 = i;
}
	...
	register int i;
{
	...
	if (i&1) { /* Fixed text */
		repr = rp[i/2];
		return !Fw_positive(repr) || allspaces(repr);
	}
	nn = child(n, i/2);
	return width(nn) == 0;
}
	...
	environ *ep;
{
	...
	case SUBRANGE:
		if (ep->s1&1)
			...
			Assert(Type(child(n, i/2)) == Tex);
			str = Str((value)child(n, i/2));
		}
		return str[ep->s2];
...
/*
 * Subroutine for focchar(), return first character of node.
 */
