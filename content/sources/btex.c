...
Visible value concat(s, t) value s, t; {
	value c;
	if (s->type != Tex || t->type != Tex) error("join with non-text");
...
Visible value reptext(x, y) value x, y; {
	value r; int i= intval(y); intlet xl= Length(x), p, q;
	string rp, xp;
...
Visible value center(x, y) value x, y; {
