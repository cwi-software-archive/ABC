...
Visible Procedure thought(c) register char c; {
	Skipsp(tx);
	if (Char(tx++) != c) syserr("I'm confused; cannot trust me own eyes");
}
...
Hidden char tagbuf[TAGBUFSIZE];
Hidden txptr tagbufend= &tagbuf[TAGBUFSIZE];

Visible value tag() {
	txptr tp= tagbuf; value line;
	Skipsp(tx);
...
Visible value findkw(u, f, t) txptr u, *f, *t; {
	txptr sp= tx, kp= tagbuf; value word= mk_text("");
...
Hidden char txbuf[TXBUFSIZE];
Hidden txptr txbufstart= &txbuf[1], txbufend= &txbuf[TXBUFSIZE];
txptr txstart, txend;
