...
bool terminated;
Hidden value global;
Hidden value formlist, sharelist; envtab reftab;
Hidden bool forming;

Hidden value mk_formal();
Hidden loc fopnd();
Hidden loc fop();
Hidden loc basfop();
Hidden Procedure unicomm_suite();
Hidden Procedure share();
Hidden Procedure sharebas();
Hidden Procedure ref_suite();
Hidden Procedure add_reftab();
Hidden Procedure adjust_unit();

Visible Procedure get_unit(filed) bool filed; {
	bool xeq0= xeq, hu= No, yu= No, tu= No;
	txptr fux= tx, lux;
	value u; literal utype, adic;
	if ((hu= atkw(HOW_TO)) || (yu= atkw(YIELD)) || (tu= atkw(TEST))) {
		lino= 1; uname= aster;
		if (cur_ilev != 0) parerr("unit starts with indentation", "");
		if (cntxt != In_load) cntxt= In_unit;
		Skipsp(tx);
		formlist= mk_elist();
		if (hu) {
...
Visible Procedure ytu_heading(name, adic, wtx, form)
 value *name; literal *adic; txptr wtx; bool form; {
	...
	*name= t2;
	if (forming && t1 != Vnil) insertl(t1, &formlist);
	if (Letter(Char(tx))) {
		t3= tag();
		if (forming) insertl(t3, &formlist);
	} else if (Char(tx) == '(') {
		if (fopnd(wtx) == Vnil) /* ignore */;
...
debug("udc^ called");
	...
	curnv= &nw_envchain;
	curnv->tab= mk_etable(); curnv->inv_env= Enil;
	cntxt= In_unit; resexp= Voi; uname= un;
	cur_ilev= 0; lino= 1;
	tx= (How_to(*aa))->fux;
	terminated= No;
debug("ready to howto");
	...
	if (!atkw(HOW_TO)) syserr("out of phase in udc");
	if (compare(uname= keyword(uux), un) != 0)
...
debug("ref_com^ called");
	...
	sv_context(&ic);
	cntxt= In_unit; resexp= Voi; /* uname= rn; */
	cur_ilev= 0; /* lino= 1; */
...
debug("ref_et^ called");
	...
	bndtglist= mk_elist(); bndtgs= &bndtglist;
	cntxt= In_unit; resexp= re; /* uname= aster; */
	cur_ilev= 0; /* lino= 1; */
...
Hidden loc basfop(q) txptr q; {
	...
	if (Char(tx) == '(') {
		...
		value t= tag();
		if (forming && !xeq) insertl(t, &formlist);
		else l= local_loc(t);
		release(t);
	} else parerr("no formal operand where expected", "");
	return l;
...
Hidden Procedure add_reftab(rt) envtab rt; {
	...
	Over (rt) {
		replace(&(curnv->tab), Key(rt, k), Assoc(rt, k), Env);
	}
}
...
Hidden Procedure adjust_unit(fux, lux, reftab) txptr *fux, *lux; value *reftab; {
	/* The text of the unit still resides in the text buffer.
	   It is moved to an allocated area and the text pointers
	   are adjusted accordingly. */
	txptr tm, ta; int adj, k; value assoc;
	ta= (txptr) getmem((unsigned)(*lux-*fux)*sizeof(*tx));
	tm= *fux; adj= ta-tm;
	while (tm <= tx) *ta++= *tm++;
	*fux+= adj; *lux+= adj;
	Over (*reftab) {
		assoc= Assoc(*reftab, k);
		(Refinement(assoc))->rp+= adj; /*Change*/
	}
}
