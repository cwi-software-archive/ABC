...
Hidden Procedure defprd();
Hidden bool is_funprd();

/*
 * Table defining all predefined functions (not propositions).
 */

Hidden struct funtab {
	char	*f_name;
	...
	{"^^", 1, 8, Dya, Other, reptext},
	{"<<", 1, 8, Dya, Other, adjleft},
	{"><", 1, 8, Dya, Other, center},
...
Visible Procedure initfprs() {
	...
	defprd("in", Dya, Pre, (txptr)In);		/* nasty */	/* what's going on here ???? */
	defprd("not'in", Dya, Pre, (txptr)Not_in);
}

Hidden Procedure defprd(repr, adic, def, fux) string repr; literal adic, def; txptr fux; {
	literal ad= adic == Zer ? FZR : adic == Mon ? FMN : FDY;
	value r= mk_text(repr), p= mk_prd(adic, def, fux, (txptr)Dummy, (value)Dummy, (bool)Dummy);
	def_unit(p, r, ad);
	release(p); release(r);
}
