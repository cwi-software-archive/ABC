 ...
 * (Purists would call it an abstract syntax tree rather than a parse tree.)
 */
...
#define Setcoord(pp, y, x, level) (Refcnt(*(pp)) == 1 || pathuniql(pp), \
	(*(pp))->p_ycoord = y, (*(pp))->p_xcoord = x, (*(pp))->p_level = level)

#define Locparent(pp) (Refcnt(*(pp)) == 1 || pathuniql(pp), &Parent(*(pp)))

#define Loctree(pp) (Refcnt(*(pp)) == 1 || pathuniql(pp), &Tree(*(pp)))

#define Addmarks(pp, x) (Refcnt(*(pp)) == 1 || pathuniql(pp), \
	(*(pp))->p_addmarks |= (x), (*(pp))->p_delmarks &= (x))

#define Delmarks(pp, x) (Refcnt(*(pp)) == 1 || pathuniql(pp), \
 ...
 * You can also hink of this as: replace(pp, n) implies noderelease(n).
